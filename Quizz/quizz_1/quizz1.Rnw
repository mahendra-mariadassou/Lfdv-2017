...
\begin{itemize}
\item $z = \cos(\theta)(\cos(\theta) + i\sin(\theta))$
\item $z = \sin(\theta)(\cos(\theta) + i\sin(\theta))$
\end{itemize}
...
\begin{itemize}
\item $z = 2i$ (en passant par la forme exponentielle / use exponential form)
\item $z = 2+i$ (en restant sous forme algébrique / use rectangular form)
\end{itemize}
